...
export default function RemoveOrderFileModal({
  open,
  setOpen,
  fileId,
  reload,
  ...
  const createFolderHttp = async () => {
    try {
      ...
      // @ts-ignore
      setLoading(true);
      const response = await AxiosProxy.delete(`/orders/remove-file/${fileId}`);
      if (response.status == 200) {
        await reload();
        setOpen(false);
        setSystemProgressContent({
          show: true,
          message: `Order File with '${fileId}' has been deleted`,
          title: 'Order File Deleted',
          success: true,
        });
      }
    } catch (error) {
      ...
      setSystemProgressContent({
        show: true,
        message: `Could not remove Order file #'${fileId}' from Order. Try again`,
        title: 'Order File Delete Fail',
        success: false,
      });
    } finally {
      setLoading(false);
